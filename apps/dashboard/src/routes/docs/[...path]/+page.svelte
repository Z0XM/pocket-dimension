...
<svelte:head>
  <title>dashboard · Docs · {artifact.title ?? artifact.sourcePath}</title>
</svelte:head>

{#if artifact.kind === "markdown"}
  <MarkdownReader html={artifact.html} title={artifact.title} sourcePath={artifact.sourcePath} />
{:else if artifact.kind === "run-folder"}
  <div class="max-w-[48rem] space-y-8 text-foreground">
    {#if artifact.primary}
      <MarkdownReader html={artifact.primary.html} title={artifact.primary.title} sourcePath={artifact.primary.sourcePath} />
    {:else}
