...
<svelte:head>
  <title>dashboard · Epic · {artifact.title ?? artifact.sourcePath}</title>
</svelte:head>
...
<div class="space-y-4">
  ...
  {#if artifact.kind === "markdown"}
    <MarkdownReader html={artifact.html} title={artifact.title} sourcePath={artifact.sourcePath} kindLabel="Epic" statusLabel={data.statusLabel} />
    ...
    <div class="max-w-[48rem] space-y-8 text-foreground">
      ...
      {:else}
        <header class="space-y-1">
          ...
          <p class="text-label tracking-widest text-muted-foreground uppercase">Epic</p>
