...
<form class="p-6 md:p-8" onsubmit={handleSubmit}>
  ...
  {#if emailNotVerified}
    <div class="mb-4 rounded-md bg-amber-500/15 p-4 text-sm">
      <p class="text-amber-700 dark:text-amber-400 mb-3">Your email address has not been verified yet.</p>
      <Button
        type="button"
        variant="outline"
        size="sm"
  ...
  <FieldGroup>
    ...
    <Field>
      <div class="flex items-center gap-2">
        ...
        <Button variant="link" class="p-0" onclick={() => (loginBy = "username")} disabled={loading}>
          <FieldLabel class="cursor-pointer {loginBy === 'username' ? 'text-primary' : 'text-muted-foreground'}" for="username-{id}"
            >Username</FieldLabel
          >
        </Button>
      </div>
    ...
    <Field>
      <div class="flex items-center">
        ...
        <a href="/forgot-password" class="ms-auto text-sm underline-offset-2 hover:underline"> Forgot your password? </a>
      </div>
