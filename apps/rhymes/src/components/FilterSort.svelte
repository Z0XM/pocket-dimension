<script lang="ts">
  ...
  function updateUrl(filters: FilterState) {
    ...
    if (filters.statuses.length === 0) {
      ...
      url.searchParams.set(
        "statuses",
        filters.statuses.map((s) => encodeURIComponent(s)).join(","),
      );
    }
    ...
    if (filters.tags.length === 0) {
      ...
      url.searchParams.set(
        "tags",
        filters.tags.map((t) => encodeURIComponent(t)).join(","),
      );
    }
  ...
  const filterKey = $derived(
    `${filterState.sort}-${filterState.statuses.join(",")}-${filterState.tags.join(",")}`,
  );
...
<div class="relative">
  ...
  {#if isOpen}
    <div
      class="absolute right-0 top-full mt-2 bg-theme-pink-2 border-2 border-theme-pink-5 p-4 z-50 min-w-[280px] max-h-[80vh] overflow-y-auto"
    >
      <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-2">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="sort"
                checked={filterState.sort === "best"}
                onchange={() => setSort("best")}
                class="accent-theme-red-1"
              />
        ...
        {#if hasActiveFilters}
          <button
            ...
            class="text-xs text-theme-red-1 hover:text-theme-red-2 underline cursor-pointer text-left"
          >
            Clear Filters
          </button>
        {/if}
