...
**`Could not resolve "../../shared/auth/vite-better-auth-ssr"`**

Same class of stale / wrong Build Path issue. `vite.config.ts` must import `@pocket-dimension/auth/vite-ssr` (workspace package), not a relative `../../shared/...` path. Clear build cache and redeploy.
