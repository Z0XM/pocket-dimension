...
<div
  class="p-4 pt-8 px-4 sm:px-8 md:px-16 md:justify-between items-center flex md:flex-row gap-4 md:gap-8"
>
  {#if !page.route.id?.startsWith("/(auth)/")}
    <div
      class="flex items-center gap-4 md:gap-8 w-full md:w-auto md:justify-start"
    >
        ...
        <div class="hidden md:flex items-end gap-2">
          {#each defaultAndFavoriteViews as view}
            ...
            >
              <Badge
                variant="outline"
                class="border-primary border-px text-primary hover:border-accent"
                >{view.name}</Badge
              >
            </a>
          {/each}
          {#if remainingViews.length > 0}
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Content
                class="bg-white/1 backdrop-blur-md"
                align="end"
              >
    ...
    <AlertDialog.Content>
      <AlertDialog.Header>
        ...
        <AlertDialog.Description>
          Are you sure you want to delete "{viewToDelete}"? This action cannot
          be undone.
        </AlertDialog.Description>
      </AlertDialog.Header>
      <AlertDialog.Footer>
