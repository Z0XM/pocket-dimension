...
<div class="p-4 pt-8 px-4 sm:px-8 md:px-16 md:justify-between items-center flex md:flex-row gap-4 md:gap-8">
  ...
  <div class="flex items-center gap-2">
    {#if isEditingName && !isDefaultView(selectedViewName)}
      <Input
        ...
        value={editedName}
        oninput={(e) => { editedName = (e.currentTarget as HTMLInputElement).value; }}
      ...
      <Button
        variant="ghost"
        size="icon"
        class="h-6 w-6 hover:text-accent"
        onclick={saveName}
        title="Save name"
      >
        ...
        {#each remainingViews as view}
          <DropdownMenu.Item class="flex items-center justify-between text-[0.625rem] gap-1 py-0 cursor-default">
            ...
            {#if !view.isDefault}
                ...
                onclick={async (e) => {
                  ...
                  try {
                    const response = await fetch(`/api/views/${encodeURIComponent(viewName)}`, {
                      method: "PUT",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ setFavorite: !isCurrentlyFavorite }),
                    });
                    if (response.ok) {
                      await fetchViews();
                      toast.success(
              ...
              >
                <StarIcon class={`size-3.5 ${view.isFavorite ? 'fill-primary' : 'fill-transparent'} group-dark:hover:fill-primary group-hover:fill-primary`} />
              </Button>
            {/if}
          </DropdownMenu.Item>
        {/each}
  ...
  <div class="hidden md:flex items-end gap-2">
    ...
    <Button
      variant="outline"
      class="text-[0.625rem] dark:hover:bg-green-500/70 dark:hover:text-secondary"
      onclick={handleSave}
    ...
    <Button
      variant="outline"
      class="text-[0.625rem] dark:hover:bg-destructive/70 dark:hover:text-secondary"
      title="Clear Filters"
      onclick={handleClear}
...
<AlertDialog.Root bind:open={deleteDialogOpen}>
  <AlertDialog.Portal>
    ...
    <AlertDialog.Content>
      ...
      <AlertDialog.Footer>
        <AlertDialog.Cancel onclick={() => { viewToDelete = null; }}>Cancel</AlertDialog.Cancel>
        <AlertDialog.Action onclick={handleDelete} class="bg-destructive text-secondary hover:bg-destructive/7x0">
          Delete
        </AlertDialog.Action>
      </AlertDialog.Footer>
    </AlertDialog.Content>
