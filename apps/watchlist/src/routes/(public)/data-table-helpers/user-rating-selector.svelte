...
<DropdownMenu.Root>
  <DropdownMenu.Trigger>
    {#snippet child({ props })}
      <Button {...props} variant="outline" class="hidden md:flex items-center gap-2 w-full sm:w-auto" disabled={isSaving}>
  ...
  <DropdownMenu.Content class="bg-white/1 backdrop-blur-md" align="end">
    <div class="p-0">
      <Input type="text" placeholder="Search users..." bind:value={searchQuery} class="mb-2" />

      {#if isLoading}
        <div class="text-center py-4 text-muted-foreground text-sm">Loading users...</div>
