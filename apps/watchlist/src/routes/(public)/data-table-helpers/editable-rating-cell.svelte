...
<EditableCellWrapper {rowId} field="my_rating" {canEdit} class={className} additionalFields={["my_infinity", "my_shitty"]} onUndo={handleRatingUndo}>
    ...
    <div class="flex items-center gap-1 justify-end">
      ...
      <div class="flex gap-0.5">
        <Button
          ...
          class="h-8 w-8 p-0 text-base {currentMode === 'infinity' ? 'border-green-500/50' : ''}"
        ...
        <Button
          ...
          class="h-8 w-8 p-0 text-base {currentMode === 'shitty' ? 'border-green-500/50' : ''}"
  ...
  {:else}
    <div class="text-end font-medium {!canRate ? 'text-muted-foreground/50' : ''}">
      {#if !canRate}
      ...
      {:else}
        <span style={getRatingColor(currentRating)}>{formatRating(currentRating)}</span>
      {/if}
    </div>
  {/if}
</EditableCellWrapper>
