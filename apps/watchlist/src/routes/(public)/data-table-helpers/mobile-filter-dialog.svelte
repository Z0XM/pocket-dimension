...
<Dialog.Root bind:open>
  <Dialog.Content class="max-w-sm max-h-[90vh] overflow-hidden flex flex-col">
    <Dialog.Header>
      ...
      <Dialog.Description>Filter by language, tags, and type</Dialog.Description>
    </Dialog.Header>

    <div class="flex-1 overflow-y-auto space-y-4 py-4">
      <!-- Language Filter -->
      <div class="space-y-2">
        <Button variant="ghost" class="w-full justify-between p-0 h-auto" onclick={() => (languageExpanded = !languageExpanded)}>
        ...
        {#if languageExpanded}
          <Input placeholder="Search languages..." bind:value={languageSearch} class="text-xs" />
          <div class="max-h-48 overflow-y-auto space-y-1 border rounded-md p-2">
            {#if filteredLanguages.length === 0}
              <div class="px-2 py-1 text-xs text-muted-foreground">No options found</div>
            {:else}
              {#each filteredLanguages as option}
                {@const isChecked = tempLanguages.includes(option)}
                <Button variant="ghost" class="w-full justify-start text-xs h-8 px-2" onclick={() => toggleLanguage(option)}>
        ...
        {#if tempLanguages.length > 0}
          <div class="flex flex-wrap gap-1">
            {#each tempLanguages as lang}
              <Button variant="outline" size="sm" onclick={() => toggleLanguage(lang)} class="h-5 text-xs rounded-full px-2">
                {lang}
      ...
      <!-- Tags Filter -->
      <div class="space-y-2">
        <Button variant="ghost" class="w-full justify-between p-0 h-auto" onclick={() => (tagsExpanded = !tagsExpanded)}>
        ...
        {#if tagsExpanded}
          <Input placeholder="Search tags..." bind:value={tagSearch} class="text-xs" />
          <div class="max-h-48 overflow-y-auto space-y-1 border rounded-md p-2">
            ...
            {:else}
              {#each filteredTags as option}
                ...
                <Button variant="ghost" class="w-full justify-start text-xs h-8 px-2" onclick={() => toggleTag(option)}>
        ...
        {#if tempTags.length > 0}
          <div class="flex flex-wrap gap-1">
            {#each tempTags as tag}
              <Button variant="outline" size="sm" onclick={() => toggleTag(tag)} class="h-5 text-xs rounded-full px-2">
                {formatOption(tag)}
                <XIcon class="size-3 ml-1" />
              </Button>
            {/each}
      ...
      <!-- Type Filter -->
      <div class="space-y-2">
        <Button variant="ghost" class="w-full justify-between p-0 h-auto" onclick={() => (typeExpanded = !typeExpanded)}>
        ...
        {#if typeExpanded}
          <Input placeholder="Search types..." bind:value={typeSearch} class="text-xs" />
          <div class="max-h-48 overflow-y-auto space-y-1 border rounded-md p-2">
            ...
            {:else}
              {#each filteredTypes as option}
                ...
                <Button variant="ghost" class="w-full justify-start text-xs h-8 px-2" onclick={() => toggleType(option)}>
                  <span class="flex-1 text-left">{formatOption(option)}</span>
                  {#if isChecked}
                    <CheckIcon class="size-4" />
                  {/if}
        ...
        {#if tempTypes.length > 0}
          <div class="flex flex-wrap gap-1">
            {#each tempTypes as type}
              <Button variant="outline" size="sm" onclick={() => toggleType(type)} class="h-5 text-xs rounded-full px-2">
                {formatOption(type)}
                <XIcon class="size-3 ml-1" />
              </Button>
            {/each}
    ...
    <Dialog.Footer class="flex flex-row gap-2 items-center justify-center">
      <Button variant="outline" onclick={handleClear} class="text-xs">Clear All</Button>
      <Button variant="outline" onclick={() => (open = false)} class="text-xs">Cancel</Button>
