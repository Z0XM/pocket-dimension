...
const suggestions = $derived.by(() => {
  if (!newTagInput.trim()) return availableTags.slice(0, 5);
  const search = newTagInput.toLowerCase();
  return availableTags.filter((t) => t.toLowerCase().includes(search)).slice(0, 5);
});
...
<EditableCellWrapper {rowId} field="tags" {canEdit} class={className} isEditedOverride={hasTagChanges} onUndo={handleUndoTags}>
  <div class="flex flex-wrap items-center gap-1">
    {#each currentTags as tag (tag)}
      ...
      {#if editMode.isEditMode && canEdit}
        ...
        >
          <Badge
            variant="outline"
            class={isNew
              ? "border-green-500/30  dark:hover:bg-green-950"
              : "border-rose-500/30  dark:hover:bg-rose-950"}
          >
      ...
      {:else}
        <!-- biome-ignore lint/a11y/useKeyWithClickEvents: clickable badge for filtering -->
        <Badge
          variant="outline"
          class="cursor-pointer border-violet-500/30  dark:hover:bg-violet-950"
        ...
        <DropdownMenu.Trigger>
          {#snippet child({ props })}
            <Button
              ...
              size="sm"
              {...props}
              class="h-6 w-6 p-0 rounded-full border-blue-500/30  dark:hover:bg-blue-950"
              title="Add tag"
            >
        ...
        <DropdownMenu.Content class="bg-white/1 backdrop-blur-md w-48" align="start">
          ...
          <div class="max-h-32 overflow-y-auto">
            ...
            {#if newTagInput.trim() && !suggestions.find((s) => s.toLowerCase() === newTagInput.toLowerCase())}
              <DropdownMenu.Item
                class="text-green-600 dark:text-green-400 cursor-pointer"
                onSelect={() => handleAddTag(newTagInput)}
              >
                Create "{newTagInput}"
              </DropdownMenu.Item>
            {/if}
