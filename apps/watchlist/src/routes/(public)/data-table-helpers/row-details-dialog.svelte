  ...
  <Dialog.Content class="max-w-sm">
    {#if row}
      ...
      <div class="space-y-4 py-4">
        ...
        {#if tagList.length > 0}
          <div class="space-y-2">
            <span class="text-[0.625rem] text-muted-foreground font-medium">Tags</span>
            <div class="flex flex-wrap gap-1.5">
              {#each tagList as tag}
                <button
                  onclick={() => handleTagClick(tag)}
                  class="text-xs cursor-pointer hover:underline capitalize px-2 py-1 rounded-md hover:bg-accent transition-colors {isTagFiltered(tag)
                    ? 'text-primary font-medium border-white border'
                    : ''}"
                >
                  {tag}
    ...
    <Dialog.Footer>
      <Button variant="outline" onclick={() => (open = false)} class="text-xs w-full">Close</Button>
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
