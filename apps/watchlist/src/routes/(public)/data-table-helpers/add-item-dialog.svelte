<script lang="ts">
  ...
  const canRate = $derived(progressStatus === "watched" || progressStatus === "dropped");
  ...
  function validateForm(): boolean {
    ...
    if (!canRate && (rating || infinity || shitty)) {
      errors.rating = "Ratings are only allowed when progress is 'Watched' or 'Dropped'";
    }
  ...
  function handleTagInputKeydown(e: KeyboardEvent) {
    if (e.key === "Enter") {
      ...
      if (tagSuggestions.length > 0 && newTagInput.trim()) {
        const match = tagSuggestions.find((s) => s.toLowerCase() === newTagInput.toLowerCase());
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const newItem = {
        ...
        rating: canRate && ratingMode === "rating" && rating ? parseFloat(rating) : undefined,
      ...
      // Success
      const successCount = (result.results?.created?.length || 0) + (result.results?.updated?.length || 0);
...
<Dialog.Root bind:open>
  <Dialog.Content class="">
    <Dialog.Header>
      ...
      <Dialog.Description>Fill in the details to add a new watch item to the list.</Dialog.Description>
    </Dialog.Header>
    ...
    <div class="space-y-4 py-4">
      ...
      <div class="space-y-2">
        ...
        <Input id="title" bind:value={title} placeholder="Enter title..." class="text-xs {validationErrors.title ? 'border-red-500' : ''}" />
      ...
      <div class="space-y-2">
        ...
        <Select.Root type="single" value={languageId} onValueChange={(v) => (languageId = v ?? "")}>
          <Select.Trigger id="language" class="w-full text-xs cursor-pointer {validationErrors.languageId ? 'border-red-500' : ''}">
            {languageId ? languageOptions.find((o) => o.value === languageId)?.label : "Select language..."}
          </Select.Trigger>
      ...
      <div class="space-y-2">
        ...
        <Select.Root type="single" value={type} onValueChange={(v) => (type = v ?? "")}>
          <Select.Trigger id="type" class="w-full text-xs cursor-pointer {validationErrors.type ? 'border-red-500' : ''}">
            {type ? typeOptions.find((o) => o.value === type)?.label : "Select type..."}
          </Select.Trigger>
          <Select.Content class="bg-white/1 backdrop-blur-md max-h-48 overflow-y-auto text-xs">
      ...
      <div class="space-y-2">
        ...
        <div class="flex flex-wrap items-center gap-1 min-h-[2.5rem] p-2 border rounded-md">
          {#each selectedTags as tag (tag)}
            <Badge variant="outline" class="rounded-full">
              <span class="text-xs">{tag}</span>
              <Button variant="ghost" size="sm" onclick={() => handleRemoveTag(tag)} class="h-4 w-4 p-0 ml-1" title="Remove tag">
                <XIcon class="size-3" />
              </Button>
            </Badge>
          {/each}
          ...
          >
            <DropdownMenu.Trigger>
              {#snippet child({ props })}
                <Button variant="ghost" size="sm" {...props} class="h-6 w-6 p-0 rounded-full border-blue-500/30" title="Add tag">
            ...
            <DropdownMenu.Content class="bg-white/1 backdrop-blur-md w-48" align="start">
              <div class="">
                ...
                <div class="max-h-32 overflow-y-auto">
                  {#if tagSuggestions.length === 0 && newTagInput.trim()}
                    <Button variant="ghost" class="w-full justify-start text-xs" onclick={() => handleAddTag(newTagInput)}>
                      <PlusIcon class="size-3 mr-2" />
                      Add "{newTagInput}"
                    </Button>
                  {/if}
                  {#each tagSuggestions as tag}
                    <Button variant="ghost" class="w-full justify-start text-xs cursor-pointer" onclick={() => handleAddTag(tag)}>
                      {tag}
                    </Button>
                  {/each}
                </div>
      ...
      <div class="space-y-2">
        ...
        <Select.Root type="single" value={progressStatus} onValueChange={(v) => (progressStatus = v ?? "")}>
          <Select.Trigger id="progress" class="w-full text-xs cursor-pointer">
            {progressStatus ? progressOptions.find((o) => o.value === progressStatus)?.label : "Select progress..."}
          </Select.Trigger>
          <Select.Content class="bg-white/1 backdrop-blur-md max-h-48 overflow-y-auto text-xs">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex gap-1">
              <Button
                ...
                class="h-8 w-8 p-0 text-base {ratingMode === 'infinity' ? 'border-green-500/50' : ''}"
              ...
              <Button
                ...
                class="h-8 w-8 p-0 text-base {ratingMode === 'shitty' ? 'border-green-500/50' : ''}"
      ...
      {:else if progressStatus}
        <div class="text-xs text-muted-foreground">Set progress to "Watched" or "Dropped" to enable rating</div>
      {/if}
    </div>
