<script lang="ts">
  ...
  import { type EditableFields, getEditModeContext, type UserRole } from "./edit-mode.svelte.js";
  ...
  function handleSelect(selectedValue: string | undefined) {
    ...
    editMode.markFieldEdited(rowId, field, valueToSave as EditableFields[typeof field], value as EditableFields[typeof field]);
    ...
    if (field === "my_progress_status") {
      const canHaveRating = valueToSave === "watched" || valueToSave === "dropped";
      if (!canHaveRating) {
...
<EditableCellWrapper {rowId} {field} {canEdit} class={className}>
  {#if editMode.isEditMode && canEdit}
    <DropdownMenu.Root>
      <DropdownMenu.Trigger>
        {#snippet child({ props })}
          <Button variant="outline" {...props} class="min-w-[120px] py-2 px-4 text-xs justify-between">
            <span class="truncate {currentValue === null || currentValue === undefined || currentValue === '' ? 'text-muted-foreground' : ''}">
              {#if field === "my_progress_status"}
      ...
      <DropdownMenu.Content class="bg-white/1 backdrop-blur-md min-w-[120px] max-h-48 overflow-y-auto" align="start">
        <DropdownMenu.RadioGroup value={currentValue ?? ""} onValueChange={handleSelect}>
  ...
  {:else}
    ...
    <span
      class="capitalize cursor-pointer hover:underline truncate {currentValue === null ? 'text-muted-foreground' : ''} {textColorMap[field]?.[
        currentValue ?? ''
      ] ?? ''}"
      onclick={handleClick}
