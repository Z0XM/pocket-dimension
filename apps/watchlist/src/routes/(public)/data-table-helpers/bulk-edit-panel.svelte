...
{#if isVisible}
  <Card.Root
    class="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 shadow-lg max-w-2xl w-[calc(100%-2rem)] px-4"
  >
    <Card.Content
      class="px-2 py-1.5 flex flex-wrap items-end justify-between gap-2"
    >
      ...
      <div class="flex flex-col gap-1 flex-1">
        ...
        <div class="flex items-center">
          <Select.Root
            type="single"
            value={bulkProgress}
            onValueChange={(v) => (bulkProgress = v ?? "")}
          >
            <Select.Trigger class="text-xs cursor-pointer">
              {bulkProgress
                ? progressOptions.find((o) => o.value === bulkProgress)?.label
                : "Select..."}
            </Select.Trigger>
            <Select.Content
        ...
        <div class="flex flex-col gap-1 flex-1">
          ...
          <div class="flex items-center">
            <Select.Root
              ...
              onValueChange={(v) => (bulkLanguageId = v ?? "")}
            >
              <Select.Trigger class="text-xs cursor-pointer">
                {bulkLanguageId
                  ? languageOptions.find((o) => o.value === bulkLanguageId)
                      ?.label
                  : "Select..."}
              </Select.Trigger>
              <Select.Content
        ...
        <div class="flex flex-col gap-1 flex-1">
          ...
          <div class="flex items-center">
            <Select.Root
              type="single"
              value={bulkType}
              onValueChange={(v) => (bulkType = v ?? "")}
            >
              <Select.Trigger class="text-xs cursor-pointer">
                {bulkType
                  ? typeOptions.find((o) => o.value === bulkType)?.label
                  : "Select..."}
              </Select.Trigger>
              <Select.Content
                class="bg-white/1 backdrop-blur-md max-h-48 overflow-y-auto text-xs"
    ...
    <Card.Footer class="flex items-center justify-between px-2">
      ...
      {#if canDelete}
        <Button
          ...
          class=" flex items-center py-2 gap-1 text-xs px-3 dark:hover:bg-rose-500 dark:bg-rose-800"
        >
