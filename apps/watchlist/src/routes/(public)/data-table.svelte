...
{#if renderTable}
  <div>
    <div class="flex items-start sm:items-center gap-3 sm:gap-0 pt-4 px-4 sm:px-8 md:px-16">
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 flex-1 min-w-0">
        <Input
          bind:ref={searchInputRef}
          id="search-input"
          placeholder={isMobile ? "Search by title..." : "(Ctrl + Q) to search by title..."}
          value={searchValue}
      ...
      <div class="flex items-center justify-end gap-2 sm:ms-auto">
        ...
        {:else if isEnteringEditMode}
          ...
          <Button variant="outline" size="icon" disabled class="flex items-center justify-center">
            <div class="size-4 border-2 border-t-transparent border-primary rounded-full animate-spin"></div>
          </Button>
        {:else}
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger>
            {#snippet child({ props })}
              <Button {...props} variant="outline" class="hidden md:flex sm:ms-auto items-center gap-2 w-full sm:w-auto"
              ...
              {#each dndItems as item (item.id)}
                ...
                {@const displayName =
                  ...
                  item.id
                    ...
                    .join(" ")}
                <div class="flex items-center gap-2 px-2 hover:bg-accent rounded-md cursor-move" data-id={item.id}>
    ...
    <div class="rounded-md relative">
      <Table.Root class="border-separate border-spacing-y-2">
          ...
          {#each table.getHeaderGroups() as headerGroup (headerGroup.id)}
            ...
            >
              {#each headerGroup.headers as header (header.id)}
                {@const isRightAligned = header.column.id === "my_rating" || header.column.id === "avg_rating"}
                <Table.Head colspan={header.colSpan} class="border-input">
                  {#if !header.isPlaceholder}
                    <div class="flex items-center gap-2 {isRightAligned ? 'justify-end' : ''}">
        ...
        <Table.Body>
          {#each table.getRowModel().rows as row (row.id)}
            ...
            >
              {#each row.getVisibleCells() as cell (cell.id)}
                {@const isCellRightAligned = cell.column.id === "my_rating" || cell.column.id === "avg_rating"}
                <Table.Cell class="border-transparent py-4 items-center {isCellRightAligned ? 'text-end' : ''}">
          ...
          {:else}
            <Table.Row>
              <Table.Cell colspan={columns.length} class="h-24 scale-[97%] text-center">No results.</Table.Cell>
            </Table.Row>
          {/each}
          {#if table.getRowModel().rows.length > 0}
            <Table.Row>
              <Table.Cell colspan={columns.length} class="p-0">
                <div bind:this={sentinelRef} class=" h-20 w-full" data-sentinel="true"></div>
              </Table.Cell>
            </Table.Row>
          {/if}
        </Table.Body>
  ...
  <DeleteConfirmationDialog bind:open={showDeleteConfirmation} titles={deleteTitles} onConfirm={handleDeleteConfirm} onCancel={handleDeleteCancel} />
  ...
  {#if isMobile}
    <RowDetailsDialog bind:open={showRowDetailsDialog} row={selectedRowForDetails} />
  {/if}
{/if}
